...
if test "$ql_prepack_opt" = "Y"; then
	if test "$ql_ext_app" = "Y"; then
		dtools pacmerge --id APPIMG,PS --id APPIMG2,PS --id PREPACK,BOOTLOADER ${version_path}/${BUILD_TARGET}_${buildver}.pac ${version_path}/app/${ql_app_ver}.pac ${version_path}/${BUILD_TARGET}_${buildver}_merge.pac
	else 
		dtools pacmerge --id APPIMG,PS --id PREPACK,BOOTLOADER ${version_path}/${BUILD_TARGET}_${buildver}.pac ${version_path}/app/${ql_app_ver}.pac ${version_path}/${BUILD_TARGET}_${buildver}_merge.pac
	fi	
else
	if test "$ql_ext_app" = "Y"; then
