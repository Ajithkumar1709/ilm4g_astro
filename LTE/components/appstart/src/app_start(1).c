...
extern bool quec_skip_spi6_ext_nor_flash_init;
    ...
    drvGpioInit();
#elif defined(__QUEC_OEM_VER_LT__) || defined(__QUEC_OEM_VER_CAP__)
...
*/
	...
	else
	{
		...
		usb_funcs[item_cnt++] = createSerialFunc(DRV_NAME_USRL_COM0); //USB AT口,用于AT/数据收发, ql_uart.h中的函数使用宏QL_USB_PORT_AT可读写此端口,linux下为ttyUSB0
		...
		usb_funcs[item_cnt++] = createSerialFunc(DRV_NAME_USRL_COM5); //USB Modem口,用于AT/数据收发,ql_uart.h中的函数中使用宏QL_USB_PORT_MODEM可读写此端口,linux下为ttyUSB5
#endif
		//由于底层端点,以及ram空间有限,因此mass storage, uac, nmea口功能三选一
		if(QL_USB_ENUM_USBNET_COM == enum_mode || QL_USB_ENUM_USBNET_COM_PRINTER == enum_mode)
		{
#ifdef CONFIG_QUEC_PROJECT_FEATURE_USB_NMEA_AT
			usb_funcs[item_cnt++] = createSerialFunc(DRV_NAME_USRL_COM6); //USB NMEA口,用于AT/数据收发,ql_uart.h中的函数中使用宏QL_USB_PORT_NMEA可读写此端口,linux下为ttyUSB6
#endif
		}
#ifdef CONFIG_QUEC_PROJECT_FEATURE_USB_AUDIO
		else if(QL_USB_ENUM_USBNET_COM_UAC == enum_mode)
		{
		...
		else if(QL_USB_ENUM_MASS_STORAGE == enum_mode)
		{
			...
			OSI_LOGI(0, "msg_dev %d", msc_cfg.protocol);
			if(msc_cfg.protocol == QL_USB_PROTOCOL_MTP)
			{
				usb_funcs[item_cnt++] = ql_usb_create_mtp();	
			}
			else
			{
				usb_funcs[item_cnt++] = ql_usb_create_msg();
			}
		}
#endif
