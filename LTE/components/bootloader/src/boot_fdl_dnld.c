...
#define PACKAGE_FILE_RESERVED_SIZE 0x8000 //预置文件时,文件系统预留32KB,如果不想预留可以改成0x0!!!!!!!!!!
...
#endif
{
    ...
    OSI_LOGXD(OSI_LOGPAR_SII, 0, "pkg file %s:%d/%u", name, isdir, f->data_size);//quectel simple log
#if 1
#if PACKAGE_FILE_RESERVED_SIZE
    //其实vfs_file_write函数中在写文件的时候也会检查可用空间是否能放下该文件,如果PACKAGE_FILE_RESERVED_SIZE改为0的情况下,可以屏蔽这部分代码
    ...
    if(f->data_size + PACKAGE_FILE_RESERVED_SIZE > fsStat.f_bavail * fsStat.f_bsize)
    {
        ret = -8;
