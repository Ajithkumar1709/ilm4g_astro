...
void ql_gnss_thread(void *param)
{
    QL_GNSSDEMO_LOG("PSTART:gnss demo thread enter, param 0x%x", param);
    ql_event_t event;
    int ret=0;
    ...
    switch(nmea->type)
    {
        ...
        case NMEA_GGA:
            ...
            if(gga != NULL)
            {
				...
				nmea_dbg_log("my Date=%d \r\n",gps_data->UTC);
				gps_data->time.tm_hour=gps_data->UTC/10000;
				nmea_dbg_log("my tm_hour=%d \r\n",gps_data->time.tm_hour);
				gps_data->time.tm_min=(gps_data->UTC/100)%100;
				nmea_dbg_log("my tm_min=%d \r\n",gps_data->time.tm_min);
				gps_data->time.tm_sec=gps_data->UTC%100;
				nmea_dbg_log("my tm_sec=%d \r\n",gps_data->time.tm_sec);


                gps_data->altitude = gga->altitude;
                nmea_dbg_log("my altitude=%f \r\n",gps_data->altitude);

                gps_data->satellites_num=gga->satellites_tracked;
				nmea_dbg_log("my satellites_num=%d \r\n",gps_data->satellites_num);

				gps_data->quality=gga->quality;
				nmea_dbg_log("my quality=%d \r\n",gps_data->quality);

				gps_data->longitude = gga->longitude.degrees + (gga->longitude.minutes / 60);
                gps_data->longitude_cardinal = gga->longitude.cardinal;
                gps_data->latitude = gga->latitude.degrees + (gga->latitude.minutes / 60);
                gps_data->latitude_cardinal = gga->latitude.cardinal;
				gps_data->hdop = gga->hdop;
				nmea_dbg_log("my hdop=%f \r\n",gps_data->hdop);
            }
        break;
        case NMEA_GSA:
...
gnssElements_t gnss_data(void) {
 extern  ql_gnss_data_t *gps_data; 	
  gnssElements_t gnssElements;
  extern ql_gnss_data_t gpsData;
  gpsData = g_gps_data;
  if ((gps_data -> valid == 1) && (gps_data -> quality == 1)) {
    gnssElements.valid = gps_data -> valid;
    gnssElements.latitude = gps_data -> latitude;
    gnssElements.longitude = gps_data -> longitude;
    gnssElements.altitude = gps_data -> altitude;
    gnssElements.hdop = gps_data -> hdop;
    gnssElements.satellites_num = gps_data -> satellites_num;
    gnssElements.quality = gps_data -> quality;
    QL_MQTT_LOG("VALID:%f,LATI:%f,LONG:%f,ALTI:%f,HFOP:%f,SAT:%d,QUALITY:%d", gnssElements.valid, gnssElements.latitude,
      gnssElements.longitude, gnssElements.altitude,gnssElements.hdop, gnssElements.satellites_num, gnssElements.quality);
  } else {
    gnssElements.latitude = gnssElements.longitude = gnssElements.altitude = gnssElements.hdop = gnssElements.satellites_num = 0;
    gnssElements.quality = 2;
    QL_MQTT_LOG("VALID:%f,LATI:%f,LONG:%f,ALTI:%f,HFOP:%f,SAT:%d,QUALITY:%d", gnssElements.valid, gnssElements.latitude,
      gnssElements.longitude, gnssElements.altitude,gnssElements.hdop, gnssElements.satellites_num, gnssElements.quality);
  }
  return gnssElements;
