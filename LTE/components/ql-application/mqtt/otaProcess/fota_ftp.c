...
extern int check_operator(uint8_t nSim,int profile_idx);
...
//FTP服务器相关信息
char hostname[] = "seepl18.net.in";         //FTP服务器 IP:端口号
char username[] = "fesppp.b";                         //FTP服务器用户名
char password[] = "spPpes.d";                         //FTP服务器用户密码
char file_full_path[] = "/ilm4g/output.pack";    //FTP服务器文件路径
...
int fota_ftp_client_init(fota_ftp_client_t *fota_ftp_cli_p)
{
	...
	if(local_file_fd <= 0)
	{
		QL_FOTA_FTP_LOG("-1-OTALOG:ftp fota loc file open failed  ret : %d!",local_file_fd);
		ret = QL_ERROR_INVALID_PARAM;
	}
...
int fota_ftp_net_register(uint8_t nSim,int profile_idx)
{
    ...
    if (QL_FTP_FOTA_SUCCESS != ret)
    {
        QL_FOTA_FTP_LOG("-1-OTALOG:network register failure!");
        return QL_ERROR_INVALID_PARAM;
    }

    ret = ql_set_data_call_asyn_mode(nSim, profile_idx, 0);
    QL_FOTA_FTP_LOG("-1-OTALOG:network datacall asyn mode ret = %d",ret);

    //ql_start_data_call(nSim, profile_idx, QL_PDP_TYPE_IP, "uninet", NULL, NULL, 0);
       check_operator(nSim, profile_idx);
     
   // ql_start_data_call(nSim, profile_idx, QL_PDP_TYPE_IP, "airtelgprs.com", NULL, NULL, 0);

    QL_FOTA_FTP_LOG("-1-OTALOG:network datacall result ret = %d",ret);
    if (0 != ret)
    {
        QL_FOTA_FTP_LOG("-1-OTALOG:data call failure!");
    }
...
int fota_ftp_server_connect(fota_ftp_client_t *fota_ftp_cli_p)
{
    ...
    QL_FOTA_FTP_LOG("-1-OTALOG:********** ql_ftp_client_open ret:%d **********",ret);
...
{
    QL_FOTA_FTP_LOG("-1-OTALOG:********** fota_ftp_get_server_file_size ***********");
    int ret = QL_FTP_FOTA_SUCCESS;
    double file_size = 0;
    ...
    QL_FOTA_FTP_LOG("-1-OTALOG:********** fota_ftp_cli_p->file_progress.total_size : %d **********", fota_ftp_cli_p->file_progress.total_size);
	return ret;
}
...
int fota_ftp_net_connect_ready(fota_ftp_client_t *fota_ftp_cli_p)
{
    ...
    if (QL_FTP_FOTA_SUCCESS != ret)
    {
        QL_FOTA_FTP_LOG("-1-OTALOG:********** Network registration failed **********");
        ret = QL_ERROR_INVALID_PARAM;
        goto exit;
    }
    ...
    {
        //未成功与FTP服务器建立连接
        QL_FOTA_FTP_LOG("-1-OTALOG:********** The connection to the FTP server was unsuccessful **********");
        ret = QL_ERROR_INVALID_PARAM;
        goto exit;
    }
    ...
    {
        //文件不存在于FTP服务器上
        QL_FOTA_FTP_LOG("-1-OTALOG:********** The file to be downloaded does not exist  or failed on the FTP server **********");
        //断开与FTP服务器的连接
    ...
    QL_FOTA_FTP_LOG("-1-OTALOG:********** residual_space_size:%d   ftp file total_size:%d**********", residual_space_size,fota_ftp_cli_p->file_progress.total_size);
    ...
    {
        //剩余空间不足,无法下载FOTA文件
        QL_FOTA_FTP_LOG("-1-OTALOG:********** There is not enough free space to download FOTA files **********");
...
void fota_ftp_show_file_progress(fota_ftp_client_t *fota_ftp_cli_p)
{
    ...
    QL_FOTA_FTP_LOG("-1-OTALOG:*************************************************************");
    QL_FOTA_FTP_LOG("-1-OTALOG:********** Current file download progress : %.1f%% **********", download_progress);
    QL_FOTA_FTP_LOG("-1-OTALOG:*************************************************************");
...
{
    QL_FOTA_FTP_LOG("-1-OTALOG:********** fota_ftp_write_cb **********");
    ...
    QL_FOTA_FTP_LOG("-1-OTALOG:ql_fwrite ret = %d",ret);
...
{
    QL_FOTA_FTP_LOG("-1-OTALOG:********** fota_ftp_get_file **********");

    int ret;

    //设置文件下载起始偏移为上次文件下载之后的位置,初始偏移量为0
    QL_FOTA_FTP_LOG("-1-OTALOG:********** downloaded_size : %ld**********", fota_ftp_cli_p->file_progress.downloaded_size);
    ...
    QL_FOTA_FTP_LOG("-1-OTALOG:********** ql_ftp_client_get_ex : %d **********", ret);
...
void fota_ftp_file_download(fota_ftp_client_t *fota_ftp_cli_p)
{
	...
	for (int i = 0; i < 10; i++)
	{
		QL_FOTA_FTP_LOG("-1-OTALOG:*************** down_state : %d ******************", fota_ftp_cli_p->down_state);
		if (DOWN_DOWNED == fota_ftp_cli_p->down_state)
		{
			//下载完成
			QL_FOTA_FTP_LOG("-1-OTALOG:FTP FOTA file download complete");
			break;
		}
		else if (DOWN_NOSPACE == fota_ftp_cli_p->down_state)
		{
			...
			QL_FOTA_FTP_LOG("-1-OTALOG:********** The object file on the server has been replaced and there is not enough space left in the FOTA package after downloading the replacement **********");
			//删除文件
			ql_remove(fota_ftp_cli_p->local_filename);
		...
		else if (DOWN_OTHER_ERROR == fota_ftp_cli_p->down_state)
		{
			...
			QL_FOTA_FTP_LOG("-1-OTALOG:********** The ql_ftp_client_get_ex function parameter configuration error, resulting in an error when downloading FTP files **********");
			break;
		}
		//FTP FOTA文件下载
...
void fota_ftp_file_check(fota_ftp_client_t *fota_ftp_cli_p)
{
    ...
    if (DOWN_DOWNED == fota_ftp_cli_p->down_state)
    {
        ...
        if (QL_FTP_FOTA_SUCCESS != ret)
        {
            QL_FOTA_FTP_LOG("-1-OTALOG:********** [%s]package is invalid FOTA **********", fota_ftp_cli_p->local_filename);
            //删除无效的FOTA文件
            ret = ql_remove(fota_ftp_cli_p->local_filename);
            fota_ftp_cli_p->down_state = DOWN_INIT;
            if (QL_FTP_FOTA_SUCCESS == ret)
            {
                QL_FOTA_FTP_LOG("-1-OTALOG:********** delete file success **********");
            }
        }
        else
        {
            //校验成功
            QL_FOTA_FTP_LOG("-1-OTALOG:********** download is sucess ,system will reset power! **********");
	...
	else 
	{
		...
		QL_FOTA_FTP_LOG("-1-OTALOG:********** download is failed ,remove file! **********");
		ret = ql_remove(fota_ftp_cli_p->local_filename);
		fota_ftp_cli_p->down_state = DOWN_INIT;
		if (QL_FTP_FOTA_SUCCESS == ret)
		{
...
ql_fota_result_e  fota_ftp_result_process(void)
{
	...
	{
		QL_FOTA_FTP_LOG("-1-OTALOG:ql_fota_get_result failed ");
		return QL_FOTA_STATUS_INVALID;
	}

	if ( p_fota_result == QL_FOTA_FINISHED )
	{
		QL_FOTA_FTP_LOG("-1-OTALOG:update finished");
		ql_fota_file_reset(TRUE);
		return QL_FOTA_FINISHED;
	}
	else if(p_fota_result == QL_FOTA_READY)
	{
		QL_FOTA_FTP_LOG("-1-OTALOG:fota ready bigen power reset ");
		ql_rtos_task_sleep_s(5);
		ql_power_reset(RESET_NORMAL);
	}
	else if(p_fota_result == QL_FOTA_NOT_EXIST)
	{
		QL_FOTA_FTP_LOG("-1-OTALOG:fota file not exist");
		return QL_FOTA_NOT_EXIST;
	}
	QL_FOTA_FTP_LOG("-1-OTALOG:fota  result status invalid");
	return QL_FOTA_STATUS_INVALID;
}


int fota_ftp_app(void)
{
    QL_FOTA_FTP_LOG("-1-OTALOG:naga inside fota ftp app init");
    //延迟启动30S
	...
	QL_FOTA_FTP_LOG("-1-OTALOG:current version:  %s", version_buf);
...
exit:
    ...
    return PASS;
init_error_exit:
    QL_FOTA_FTP_LOG("-1-OTALOG:********** exit ql_ftp_fota_demo **********");
    return FAIL;
}
