...
extern uint32_t randomTimeAstroSend;
extern uint8_t astroSendFlage;
   ...
   /* if (rtc.status == RTC_SET && rtc.daySecs >= 54000 && rtc.daySecs <= 54006) {
    ...
    if (astroSendFlage) {
      if (rtc.status == RTC_SET) {
       if (rtc.daySecs >= randomTimeAstroSend) {
         ilmData.pktType = D_ASTRO;
         ilmData.dataSize = 1;
         ql_rtos_queue_release(ql_ilmProcess_qHandle, sizeof(ilmData), (uint8 * ) & ilmData, 0);
         astroSendFlage = false;
         QL_MQTT_LOG("-1-ONBOOTLOG:D_ASTRO release in 5.30PM");
       }
      }
   }
